...
public class ActorBuffManager : MonoBehaviour
{
    ...
    private void HandleOwnerPerceive(Actor buffOwner)
    {
        // scheduled remove
        for (int i = _toBeRemove.Count - 1; i >= 0; i--)
        {
            ...
            entry.StateMachine.SendMessage(MESSAGE_REMOVE);
        }
        ...
        for (int i = _toBeAdd.Count - 1; i >= 0; i--)
        {
            ...
            _toBeAdd.RemoveAt(i);
            entry.StateMachine.SendMessage(MESSAGE_ADD);
        }
    }

    private void HandleOwnerDie(Actor buffOwner)
    {
        // cleanup
    ...
    public class Birth : State<ManagedEntry, Birth>
    {
        ...
        public override void OnMessage(ManagedEntry ctx, string message)
        {
            switch (message)
            {
                case MESSAGE_ADD:
